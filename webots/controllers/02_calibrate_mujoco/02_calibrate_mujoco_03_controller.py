...
def webots_robot():
    ...
    for i in range(N):
        ...
        # getColors, getModel, getNumberOfColors, getOrientation, getPosition, getPositionOnImage, getSize, getSizeOnImage

        found = []
        for obj in camera.getRecognitionObjects():
            #print('{:.2f}, {}, {}'.format(robot.getTime(), tuple(obj.getPositionOnImage()), tuple(obj.getSizeOnImage())))
            # getRangeImage(data_type='list'), getRangeImageArray(), rangeImageGetDepth(image, width, x, y)
            width = 320
            x, y = obj.getPositionOnImage()
            z = rangefinder.rangeImageGetDepth(rangefinder.getRangeImage(), width, x, y)
            found = (x,y,z)

        # Want the x,y,z position of object wrt camera
        print('{:.2f}, {}'.format(robot.getTime(), found))
        # 5.73, (165, 141, 0.35433778166770935)
...
<mujoco>
   <compiler autolimits="true"/>
    ...
    <worldbody>
        ...
        <body name="ball" pos="0.43 0 0">
          <freejoint/>
          <geom type="sphere" size="0.01" rgba=".5 .5 1 1"/>
        </body>
